...
function main()
    if help
        print(stdout, HelpMessage(join([str_julia, basename(@__FILE__()), str_sis], " "), str_description["recovery"], [Block(switchtitle, str_switch["recovery"])]))
        return
    end
    if isempty(input)
    ...
    recovery = recovery_report(data; pre, post, type, pct, colanalyte, colstats, collevel)
    td = merge ? pivot(selectby(recovery, colstats, pct ? ["Recovery(%)", "Standard Deviation(%)"] => mean_plus_minus_std => "Recovery(%)" : 
                                                    ["Recovery", "Standard Deviation"] => mean_plus_minus_std => "Recovery"), 
                        cols; rows, notsort, drop, prefix) : pivot(recovery, cols; rows, notsort, drop, prefix)
    println(stdout)
    display(td)
    println(stdout)
    i = 0
    ...
    mkpath(file)
    CSV.write(joinpath(file, "raw.csv"), recovery)
